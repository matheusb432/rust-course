    ...
    (
        ...
        $(
            $config.add_section(stringify!($section));
            {
                {
                    $(
                        // ? Will disallow duplicate keys in a section
                        #[allow(warnings)]
                        struct $key;
                    )+
                }
...
fn main() {
    ...
    // NOTE Acessing a module constant created by a macro
    // ? This is a compile-time constant, so performance is not affected
