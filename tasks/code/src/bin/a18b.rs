...
//
// Requirements:
// * 1. Write the body of the `authorize` function. The steps to authorize a user are:
...
// * 2. Run the program after writing your `authorize` function. Expected output:
//     Ok(Allow)
//     Ok(Deny)
//     Err("Catherine doesn't have a keycard")
// * 3. Use the question mark operator within the `authorize` function.
//
// Notes:
...
fn authorize(
    ...
    // * 3.
    // NOTE if at any point in these 3 expressions a Err() result is returned, the function will exit with the error result
